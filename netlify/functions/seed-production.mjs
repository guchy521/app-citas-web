...
export default async (req) => {
  ...
  try {
    const [negocio] = await sql`
      ...
      ON CONFLICT (slug) DO UPDATE SET admin_pin_hash = EXCLUDED.admin_pin_hash
      RETURNING id
    `;

    const yaHayServicios = await sql`SELECT id FROM servicios WHERE negocio_id = ${negocio.id} LIMIT 1`;
    if (yaHayServicios.length === 0) {
      await sql`
