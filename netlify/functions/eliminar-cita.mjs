...
export default async (req) => {
  ...
  try {
    ...
    if (!id) {
      return new Response(JSON.stringify({ exito: false, mensaje: "Falta el id de la cita." }), { status: 400, headers: { "Content-Type": "application/json" } });
    }

    const resultado = await sql`
      DELETE FROM citas WHERE id = ${id} AND negocio_id = ${sesion.negocio_id}
    `;
