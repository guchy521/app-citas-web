...
export default async (req) => {
  ...
  try {
    const servicios = await sql`
      SELECT id, nombre, precio, duracion_horas, descripcion
      FROM servicios
      WHERE negocio_id = ${sesion.negocio_id} AND activo = true
      ORDER BY nombre
    `;

    return new Response(JSON.stringify({ ok: true, servicios }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
  } catch (err) {
